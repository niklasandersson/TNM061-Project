...
void main()
{
	...
	float materialAlpha = texture2D( AlphaTextureSampler, UV + 0.01*Time).r;
	vec3 materialDiffuseColor = texture2D( DiffuseTextureSampler, UV + 0.01*Time).rgb;
	...
	vec3 l[10];
	float diffuseAngle[10];
	float specularAngle[10];
	float distance[10];
	...
	color.xyz = (1) * (materialAmbientColor);
