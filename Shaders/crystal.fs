...
void main()
{
	...
	float tmp = clamp(abs(sin(Time)),0.3,0.8);
	vec3 materialDiffuseColor = texture2D( DiffuseTextureSampler, UV + Time).rgb*tmp;
	
	vec3 materialAmbientColor = vec3(0.5,0.5,0.5) * materialDiffuseColor;
	vec3 materialSpecularColor = vec3(1.0,1.0,1.0);
	float materialSpecularIntensity = 3;
	float materialSpecularFaktor = 7;
	...
	color.xyz = (1) * (materialAmbientColor+  (diffuse + specular));
